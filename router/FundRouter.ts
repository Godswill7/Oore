...
// router.route("/:userID/create-beg").post(uploadData.single("image"), createBeg)
router.route("/:userID/create-beg").post(createBeg)
router.route("/view-beg").get(viewBeg)

router.route("/:beggerID/:begID/:giverID/give-beg").patch(giveOneBeg)
...
router.route("/:userID/:begID/like-beg").patch(likeBeg)
